...
from raw_single_zhijie import BPGNN
import os
...
data_frame = pd.read_csv('shou_zhanxiang_bishe_zhijie_input_8000.csv', header=None, engine='python',encoding = 'gb2312')#header=None是无表头的意思，header=0添加表头即第一行，列没有表头
...
# print(dataset)
label_frame = pd.read_csv('shou_zhanxiang_bishe_zhijie_output_8000_chu0.csv', header=None, engine='python',encoding = 'gb2312')
labelset = label_frame.values
# print(labelset)
labelset = scaler1.fit_transform(labelset)
# print(labelset)
x_train, x_test, y_train, y_test = train_test_split(dataset, labelset, test_size=0.3, shuffle=True, random_state=19)#划分数据集，并打乱（但每次拆分的结果是相同的）
# print(y_train)
...
train_loader = DATA.DataLoader(dataset=train_data,batch_size=700)#batch_size一批样本的大小， train_loader的作用就是把所有数据分成几个批次，对每个批次进行训练
...
def nettrain (para_epoch): #执行完一次前向传播和反向传播叫做一个epoch
    ...
    lrr = torch.optim.lr_scheduler.StepLR(optimizer,step_size=400, gamma=0.1)#创建一个学习率调度器（StepLR 调度器），首先一个大前提是需要自己定义一个model和一个optimizer, 最后再将 optimizer 放入 lr_scheduler 中（前两行），
...
if __name__ == '__main__':
    ...
    for i in range(1): #开始一个循环，迭代5次(0-4)
        ...
        if mae_train+mae_test<minloss:
            ...
            bestnet=subGNN
    print("运行5次平均测试误差为：",np.mean(test_mae))#np.mean()是NumPy库的函数，用于计算数组中所有元素的平均值
    torch.save(bestnet,"123234523423.pkl")
